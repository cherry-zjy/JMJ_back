<template>
  <div>
          ...
          <el-table :data="list" style="width: 100%;margin:60px 0" :border='true'>
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button size="mini" plain icon="el-icon-edit" @click="Edit(scope.$index)">修改</el-button>
    ...
    <el-dialog title="新增配送区域" :visible.sync="dialogFormVisible" width="50%">
      <el-form :model="addForm" :rules="listrules" ref="addForm" label-width="150px" class="demo-editForm"
        ...
        <el-form-item label="配送区域" prop="Provinces">
          ...
          <el-select v-else multiple v-model="addForm.Provinces" placeholder="请选择">
            <el-option v-for="item in this.Address" :key="item.label" :label="item.label" :value="item.label">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="运费" prop="FirstPrice">
          <el-input v-model="addForm.FirstPrice"></el-input>
        </el-form-item>
        <el-form-item label="续件" prop="Per">
          <el-input v-model="addForm.Per"></el-input>
        </el-form-item>
        <el-form-item label="续费" prop="PerPrice">
          <el-input v-model="addForm.PerPrice"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改配送区域" :visible.sync="dialogFormVisible1" width="50%">
      <el-form :model="editForm" :rules="listrules" ref="editForm" label-width="150px" class="demo-editForm"
        label-position="left">
        <el-form-item label="配送区域" prop="Provinces">
          <i v-if="!isshow" class="el-icon-loading"></i>
          <el-select v-else multiple v-model="editForm.Provinces" placeholder="请选择">
            <el-option v-for="item in this.Address1" :key="item.label" :label="item.label" :value="item.label">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="首件" prop="First">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitForm1('editForm')">修 改</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.list.push({
            Provinces: this.addForm.Provinces.join(','),
            First: this.addForm.First,
            FirstPrice: this.addForm.FirstPrice,
            Per: this.addForm.Per,
            PerPrice: this.addForm.PerPrice
          });
    ...
    submitForm1(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          // 删除当前修改的原数据 并插入修改后的数据
          this.list.splice(this.submintIndex, 1, {
            Provinces: this.editForm.Provinces.join(','),
    ...
    Edit(index) {
      ...
      this.submintIndex = index;
      this.editForm = this.list[index];
      this.editForm.Provinces = this.editForm.Provinces.split(',')
    },
    submitFormwork(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$http
            .post(
              "api/Back_CurrentManage/FreightFormworkEdit",
              qs.stringify({
                token: getCookie("token"),
                id: window.location.href.split("id=")[1],

      ...
      <el-form :model="editlist" :rules="rules" ref="editlist" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="地区" prop="city">
          <el-select size="small" style="width: 100px" v-model="selectProv" placeholder="请选择省份" v-on:change="getProv($event)">
            <el-option v-for="item in provs" :key="item" :label="item.label" :value="item.value"></el-option>
          </el-select>
          <el-select size="small" style="width: 100px" v-if="selectProv!=''" v-model="selectCity" placeholder="请选择城市"
            v-on:change="getCity($event)">
            <el-option v-for="item in citys" :label="item.label" :value="item.value" :key="item"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      ...
      submitaddForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http
              .post("api/Back_UserManage/CityAdd?token=" + getCookie("token") + "&id=" + window.location.href.split(
                  "id=")[1] + "&phone=" + this.editlist.phone +"&city="+this.selectCity,
                // qs.stringify({
                //   token: getCookie("token"),
